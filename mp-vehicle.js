// mp-vehicle.js v2.7.0 — No maneuver dropdown, maneuver from per-system adjMan only
...
class Vehicle {
  ...
  addSystem() {
    const sys = {
      ...
      cells: [],
      integral: false, bulky: 0, delicate: 0,
      adjST: 0, adjEN: 0, adjAG: 0, adjIN: 0, adjCL: 0, adjMan: 0,
    };
  ...
  get turnRate() {
    let tr = 3;
    for (const s of this.systems) tr += (s.adjMan || 0);
    return tr;
  }
  ...
  toJSON() {
    return {
      version: 7, type: "mp-vehicle",
      name: this.name, model: this.model, operator: this.operator,
      basicCost: this.basicCost, techMod: this.techMod,
      wontExplode: this.wontExplode, isBase: this.isBase,
      notes: this.notes, systems: this.systems,
  ...
  fromJSON(data) {
    ...
    this.systems = (data.systems || []).map(s => {
      ...
      s.adjCL = s.adjCL || 0; s.adjMan = s.adjMan || 0;
      return s;
    });
  ...
  toCSV() {
    ...
    rows.push([]);
    rows.push(["Cost","Spaces","Profile","Hits","PTs","DMG","Description","Int","Blk","Del","ST","EN","AG","IN","CL","Man"]);
    for (const s of this.systems) {
      ...
      rows.push([s.extraCPs||0, s.spaces, this.sysProfileDisplay(s), this.sysHits(s),
        ...
        s.integral?1:0, s.bulky||0, s.delicate||0,
        s.adjST||0, s.adjEN||0, s.adjAG||0, s.adjIN||0, s.adjCL||0, s.adjMan||0]);
    }
