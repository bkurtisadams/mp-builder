// mp-vehicle.js v2.6.0 — Per-system modifiers and BC adjustments
...
class Vehicle {
  constructor() {
    ...
    this.systems = [];
    this.keyEntries = [];
    this.pictureData = ""; // base64 data URL
    this._nextId = 1;
    this._nextKeyId = 1;
  }
  ...
  addSystem() {
    const sys = {
      ...
      dmg: "",
      cells: [],
      // Per-system modifiers
      integral: false,
      bulky: 0,
      delicate: 0,
      // BC adjustments (add to vehicle stats)
  ...
  // CPs generated by system spaces (from the table)
  sysBaseCPs(sys) {
    if (!sys.spaces) return 0;
    return MP.lookupSys(sys.spaces).cp;
  }

  // Total CPs for the system (base + extra + tech + modifier adjustments)
  sysCPs(sys) {
  ...
  get totalCost() {
    let cost = this.baseCost + this.systemExtraCPs + this.techMod + this.maneuverMod;
    if (this.wontExplode) cost += 5;
    // Bulky/delicate cost adjustments
  ...
  get turnRate() {
    let tr = 3 + (this.maneuverMod / 5);
    for (const s of this.systems) tr += (s.adjMan || 0);
    return tr;
  }
  get armor() {
    return { kin: 3, eng: 3, bio: 3, ent: 3, psy: 0 };
  }
  get explosionDice() {
