// mp-canvas.js v1.0.0 — Floor plan canvas editor
// Adapted from SchematicPlannerApp pattern (gc-map-tools)

const CELL_PX = 28;  // pixels per system space at zoom=1
...
class FloorPlanEditor {
  constructor(canvasEl, wrapEl, vehicle) {
    ...
    this.pickType = null;   // system type id to place
    this.placeSpaces = 1;   // spaces for next placement
    this.placeW = 1;        // grid width for placement
    this.placeH = 1;        // grid height for placement

    this.selected = null;   // system id
    this.dragging = null;   // { id, offGx, offGy }
    this.ghost = null;      // { gx, gy, gw, gh, typeId }
    ...
    this.onUpdate = null;   // callback when systems change
  ...
  _calcPlaceSize() {
    // Find best rectangle for the space count
  ...
  _systemAt(cx, cy) {
    ...
    for (let i = this.veh.systems.length - 1; i >= 0; i--) {
      ...
      if (gx >= s.gx && gx < s.gx + s.gw && gy >= s.gy && gy < s.gy + s.gh) {
        return s;
      }
    }
    return null;
  ...
  draw() {
    ...
    for (let gx = startGx; gx <= endGx; gx++) {
      ctx.strokeStyle = gx === 0 ? "#d0482040" : gx % 4 === 0 ? "#2a2a40" : "#1a1a28";
      ctx.lineWidth = gx === 0 ? 1.5 : gx % 4 === 0 ? 0.8 : 0.4;
      const x = ox + gx * cell;
      ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke();
    }
    for (let gy = startGy; gy <= endGy; gy++) {
      ctx.strokeStyle = gy === 0 ? "#d0482040" : gy % 4 === 0 ? "#2a2a40" : "#1a1a28";
      ctx.lineWidth = gy === 0 ? 1.5 : gy % 4 === 0 ? 0.8 : 0.4;
      const y = oy + gy * cell;
      ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke();
    }

    // Movement space labels (every 4 cells = 1 movement space = 5')
    ...
    for (let gx = 0; gx <= endGx; gx += 4) {
      const x = ox + gx * cell;
      if (x > 0 && x < W) ctx.fillText(`${gx}`, x + 2, 2);
    }
    ctx.textBaseline = "middle";
    for (let gy = 4; gy <= endGy; gy += 4) {
      const y = oy + gy * cell;
      if (y > 0 && y < H) ctx.fillText(`${gy}`, 2, y);
    }
    ctx.textBaseline = "alphabetic";

    // Placed systems
    for (const s of this.veh.systems) {
      this._drawSystem(ctx, s, s.id === this.selected, cell, false, ox, oy, dpr);
    }
  ...
  _drawSystem(ctx, s, selected, cell, isGhost, ox, oy, dpr) {
    ...
    ctx.fillStyle = type?.color || "#666";
    ctx.fillRect(x + 1, y + 1, w - 2, h - 2);

    // Border
    ctx.strokeStyle = selected ? "#ffd060" : "#00000088";
  ...
  toImage(maxWidth) {
    // Find bounding box of all placed systems
    ...
    // Grid
    for (let x = 0; x <= gw; x++) {
      ctx.strokeStyle = x % 4 === 0 ? "#2a2a40" : "#1a1a28";
      ctx.lineWidth = x % 4 === 0 ? 0.8 : 0.4;
      ctx.beginPath(); ctx.moveTo(x * cellPx + 1, 0); ctx.lineTo(x * cellPx + 1, h); ctx.stroke();
    }
    for (let y = 0; y <= gh; y++) {
      ctx.strokeStyle = y % 4 === 0 ? "#2a2a40" : "#1a1a28";
      ctx.lineWidth = y % 4 === 0 ? 0.8 : 0.4;
